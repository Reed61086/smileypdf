...
# Determine if this is a Hindi page and set the path prefix accordingly
$isHindi = $FilePath -match "/hi/" -or $FilePath -match "\\hi\\"
$pathPrefix = ""
if ($isHindi) {
    $pathPrefix = "../"
}
...
$headPattern = "</head>"
$darkModeCssLink = "<link rel=`"stylesheet`" href=`"${pathPrefix}dark-mode.css`">`n</head>"
$content = $content -replace $headPattern, $darkModeCssLink

# Add dark mode JS script before the closing body tag
$bodyPattern = "</body>"
$darkModeJsScript = "<script src=`"${pathPrefix}dark-mode.js`"></script>`n</body>"
